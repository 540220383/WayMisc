...
-(void)setPlayingMusic:(BroadcastingModel *)playingMusic{
    _playingMusic = playingMusic;
    
    //歌手的头像,歌手的头像是圆形，有边框
    UIImage *cicleImage = [UIImage circleImageWithName:playingMusic.img_url borderWidth:1.0 borderColor:[UIColor yellowColor]];
    self.singerImgView.image = cicleImage;
    self.singerImgView.layer.cornerRadius = 29;
    self.singerImgView.clipsToBounds = YES;
    //歌曲名
    self.musicNameLabel.text = playingMusic.muc_name;
    //歌手名
    self.singerNameLabel.text = playingMusic.user_name;
...
- (IBAction)playBtnClick:(UIButton *)btn {
    ...
    if (self.playing) {//播放音乐
        ...
        [btn setNormalImage:@"footplayer_play"];
        
        [self notifyDelegateWithBtnType:BtnTypePlay];
        }else{//暂停音乐
        NSLog(@"暂停音乐");
        //2.如果当前是暂停的状态，按钮的图片更改为播放的状态
        [btn setNormalImage:@"footplayer_pause"];
